...
export class UsersService {
  ...
  obtainUser(id: number) {
    return this.http.get<any[]>(this.Url + '/' + id);
  }

  updateUser(user: User) {
    return this.http.put<any>(this.Url, JSON.stringify(user), this.httpOptions)
  }

  addUser(user: User) {
    return this.http.post<any>(this.Url, JSON.stringify(user), this.httpOptions)
  }

  deleteUser(id: Number, token: string) {
    const httpOptionsAuthentication = {
